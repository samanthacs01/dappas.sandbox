...
func binding[TInput any](ctx *gin.Context, input *TInput) bool{
	if err := ctx.ShouldBindUri(&input); err != nil {
		problem := BadRequest(err)
		data, ok := problem.Data.(problemDetails)
		if ok {
			data.Instance = ctx.Request.RequestURI
		}
		problem.Data = data
...
func GinHandler[TInput any](handler func(*TInput) Result) gin.HandlerFunc {
	return func(ctx *gin.Context) {
		var input TInput
		// Initialize input using reflection
		initializeInput(&input)
		if !binding(ctx, &input) {
			return
		}
		result := handler(&input)
		processHandlerResult(ctx, result)
	}
}

func GinHandlerWithWriter[TInput any](handler func(*TInput, gin.ResponseWriter) Result) gin.HandlerFunc {
	return func(ctx *gin.Context) {
		var input TInput
		// Initialize input using reflection
		initializeInput(&input)
		...
		result := handler(&input, ctx.Writer)
		...
		if result.IsFailed() {
			...
			result.Data = data
			ctx.AbortWithStatusJSON(result.Status, result)
			return
		}
	}
}

func GinHandlerParamsLess(handler func() (Result)) gin.HandlerFunc {
	return func(ctx *gin.Context) {
		result := handler()
		processHandlerResult(ctx, result)
	}
...
func processHandlerResult(ctx *gin.Context, result Result) {
	if result.IsFailed() {
		
		problem, ok := result.Data.(problemDetails)
		if !ok {
			problem.Instance = ctx.Request.RequestURI
		}
		ctx.AbortWithStatusJSON(result.Status, problem)
		return
	}
