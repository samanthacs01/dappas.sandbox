...
func init() {
	goose.AddMigrationContext(upFixAcceptInvoices, downFixAcceptInvoices)
}

func upFixAcceptInvoices(ctx context.Context, tx *sql.Tx) error {
	acceptProcedureStoreDDL := `
		...
		$$ LANGUAGE plpgsql;
	`
	_, err := tx.Exec(acceptProcedureStoreDDL)
	return err
}

func downFixAcceptInvoices(ctx context.Context, tx *sql.Tx) error {
