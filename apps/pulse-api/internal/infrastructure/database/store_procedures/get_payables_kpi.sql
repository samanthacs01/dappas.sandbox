DROP FUNCTION get_payables_kpi(p_start_date date, p_end_date date);
CREATE OR REPLACE FUNCTION get_payables_kpi(p_start_date date, p_end_date date)
			RETURNS table (
			...
			BEGIN
				total_outstanding := SUM(balance)
					FROM bills
					WHERE created_at BETWEEN p_start_date AND p_end_date AND status <> 'paid' and deleted_at is null;
				if total_outstanding is null then 
				   total_outstanding := 0;
				end if;
				total_overdue := SUM(balance)
				     FROM bills
				     WHERE created_at BETWEEN p_start_date AND p_end_date and due_date < CURRENT_DATE
				     AND status <> 'paid';
				...
				_total_paid_amount := SUM(b.amount) FROM bills b
					...
					where ip.last_payment_date::date BETWEEN p_start_date AND p_end_date and b.status = 'paid' and ip.last_payment_date::date <= b.due_date AND b.deleted_at is null;
				...
				_total_due_amount := sum(amount) from bills where created_at <= p_end_date and deleted_at is null;
				...
				production_payment_on_uncollected_invoices := (with payment_bills as (
						select distinct bp.id as payment_id, bp.amount
						from bills b 
						...
						where i.status <> 'paid' and b.deleted_at is null and f.deleted_at is null and bp.payment_at::date between p_start_date AND p_end_date
					)
