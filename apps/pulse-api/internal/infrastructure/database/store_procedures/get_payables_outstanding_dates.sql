DROP FUNCTION IF EXISTS get_payables_outstanding_dates(p_from date, p_to date);
CREATE OR REPLACE FUNCTION get_payables_outstanding_dates(p_from date, p_to date)
...
AS $$
	...
	cte_bills_by_dates as ( 
  	...
  	cte_time_frames_with_total as (	
	 	select tf.grouping, sum(case when b.balance = null then 0 else b.balance end) as total, tf.day as to_order 
		from get_grouping_from_dates(p_from, p_to) tf 
		left join cte_bills_by_dates b on tf.day = b.created
